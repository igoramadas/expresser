...
var Assets = (module.exports = function(Mincer, options) {
    if (!options) {
        options = Mincer
        Mincer = require("./mincer.js")
    }
    this.options = options
    if (this.options.compile) {
        this.environment = new Mincer.Environment()
        this.environment.ContextClass.defineAssetPath(
...
Assets.prototype.compile = function(callback) {
    ...
    if (this.manifest) {
        ...
        this.environment.eachLogicalPath(this.options.precompile, function(path) {
            try {
                instance.environment.findAsset(path)
            } catch (err) {
...
Assets.prototype.serveAsset = function(req, res, next) {
    ...
    try {
        ...
        if (this.manifest && this.manifest.assets[parts.path]) {
            ...
            asset.contentType = mime.getType(pathModule.join(this.options.buildDir, assetName))
        }
    } catch (err) {
        return next(err)
    }
...
Assets.prototype.helper = function(tagWriter, ext) {
    ...
    return function(path, options) {
        ...
        asset = instance.getAssetByPath(path, {
            bundle: true
        })
        if (!asset) {
