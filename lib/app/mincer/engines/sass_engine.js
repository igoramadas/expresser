...
var logger = require("../../../logger.coffee");
...
SassEngine.prototype.sassImporter = function(context, url, prev) {
    ...
    if (isFileNotFound(firstError) && isFileNotFound(secondError)) {
        logger.warn(util.format('Mincer.sass', '%s will not change when %s changes, because the file could not be found.', prev, url, firstError.message, secondError.message));
    }
...
function isSassType(obj) {
    for (var type in sass.types) {
        if (sass.types.hasOwnProperty(type)) {
            if (obj instanceof sass.types[type]) {
                return true;
            }
        }
    }
