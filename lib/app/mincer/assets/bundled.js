...
var BundledAsset = module.exports = function BundledAsset() {
    ...
    processedAsset = this.environment.findAsset(this.pathname, {
        bundle: false
    });
    ...
    // SOURCE MAPS TEMPORARILY DISABLED!
    // TODO! Re-enable source maps support working with Uglify ES.
    if (true || !this.environment.isEnabled('source_maps')) {
    ...
    this.mtime = _.chain(this.toArray().concat(this.__dependencyPaths__))
        .map(function(asset) {
            return new Date(Date.parse(asset.mtime));
        })
        .max()
        .value();
...
BundledAsset.prototype.initWith = function(environment, hash) {
    ...
    prop(this, '__processedAsset__', environment.findAsset(this.pathname, {
        bundle: false
    }));
