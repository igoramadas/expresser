...
var Manifest = module.exports = function Manifest(environment, pathname) {
    ...
    if (fs.existsSync(this.path)) {
        try {
            ...
            logger.error("Mincer.manifest", this.path + ' is invalid: ' + err);
        }
    }
...
Manifest.prototype.compile = function(files, options) {
    ...
    paths.forEach(function(logicalPath) {
        ...
        logger.debug('Mincer.sass', format('Compiled %s (%dms)', logicalPath, timer.stop()));
        ...
        if (fs.existsSync(target)) {
            logger.debug('Mincer.sass', 'Skipping ' + target + ', already exists');
            self.save();
            return;
        }
        ...
        self.save();
        logger.info('Mincer.manifest', 'Writing ' + target);
    });
