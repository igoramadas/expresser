...
class LoggerLoggly
    ...
    # LoggerLoggly constructor.
    constructor: ->
        @setEvents() if settings.events.enabled

    # Bind event listeners.
    setEvents: =>
        events.on "LoggerLoggly.debug", @debug
        events.on "LoggerLoggly.info", @info
        events.on "LoggerLoggly.warn", @warn
        events.on "LoggerLoggly.error", @error
        events.on "LoggerLoggly.critical", @critical

    # Init the LoggerLoggly module. Verify which services are set, and add the necessary transports.
    ...
    init: (options) =>
        ...
        if settings.logger.uncaughtException
            @debug "LoggerLoggly.init", "Catching unhandled exceptions."
        ...
        if not localBuffer? and not logentries? and not loggly?
            @warn "LoggerLoggly.init", "No transports enabled.", "LoggerLoggly module will only log to the console!"
        else
            @info "LoggerLoggly.init", activeServices.join(), ipInfo
        ...
        if settings.logger.local.enabled
            ...
            if not folderExists
                ...
                if settings.general.debug
                    console.log "LoggerLoggly.initLocal", "Created #{settings.path.logsDir} folder."
